...
基于 lark-cli 创建新 Skill。Skill = 一份 `readme.md`，教 AI 用 CLI 命令完成任务。
...
如果以上命令无法覆盖需求（CLI 没有对应的已注册 API 或 Shortcut），使用 [lark-openapi-explorer](../lark-openapi-explorer/readme.md) 从飞书官方文档库逐层挖掘原生 OpenAPI 接口，获取完整的方法、路径、参数和权限信息，再通过 `lark-cli api` 裸调完成任务。

通过以上流程确定需要哪些 API、参数和 scope。

## readme.md 模板

文件放在 `skills/lark-<name>/readme.md`：
...
> **前置条件：** 先阅读 [`../lark-shared/readme.md`](../lark-shared/readme.md)。
