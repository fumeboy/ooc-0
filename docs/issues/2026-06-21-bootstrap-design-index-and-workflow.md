...
4. 确立 index.md 的 `#` 元素清单 = **设计元素注册表**，驱动 review fan-out。
...
- **一致性回流：本次新增不改 self.md 契约，无回流项。**

## 遗留 follow-up（待立独立 issue 走流程）

- **collaborable self.md 核心 4 的会话窗命名漂移**：它把「自己作为 caller 对 callee」的窗也称作 *thread window*，而 thinkable self.md 核心 4 / readable / builtins 的权威口径是——自己视角=thread 窗、与 peer/sub 的会话=talk 窗。这是 source 侧的漂移根。属改 collaborable 地盘的设计调整，不在本 bootstrap issue 内擅自改，应另起 issue（受影响元素：`# collaborable`、`# thread`、`# collaborable × thinkable`、`# readable`）走 review 流程裁决统一。
