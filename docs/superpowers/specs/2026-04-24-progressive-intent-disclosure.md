...
父 thread 看到：
  ...
  submit(talk) {
    process_summary: {
      loaded_traits: [user_relations, talkable_delivery, verifiable],
      executed_actions: [
...
注意 "open(read ...)" 这一项**不附带任何来自内层 sub thread 的 process_summary**——内层加载了什么 trait、执行了什么 action，留在内层自己的 writeback 记录里。每层只描述自己的过程，不向上递归、不向下展开。每多一层嵌套，多一层折叠，外部干净度不变。
...
5. **submit / close 记录的粒度（含过程概述）**：sub thread 终止时写回 parent 的记录不仅是结果，还要带 sub thread 过程的**结构化概述**：
   ...
   ```text
   {
     ...
     process_summary: {
       loaded_traits: [trait_name_1, trait_name_2, ...],        // 过程中加载的 trait 名（去重）
   ...
   **字段来源与归并规则**：
   ...
   - `loaded_traits` 的标题就是 **trait name 本身**（trait 文件/注册表中的名字），无需额外 title 字段。
   ...
   - **内层 sub thread 的 process_summary 不向外蔓延**：嵌套 open 时，外层的 executed_actions 里只出现"open(inner_action)"这一条，内层自己的 loaded_traits / executed_actions 留在内层的 writeback 记录里，不递归进入外层。每层只描述自己的过程，不重复祖先或子层的内容。
...
| 存量机制 | 归并方式 |
|---|---|
| Phase 0 起点：系统协议基座 / 显式 pin | Activator 的"起点源"输入 |
...
```text
inputs:
  origin:   always-on protocol traits + explicit pinned traits
