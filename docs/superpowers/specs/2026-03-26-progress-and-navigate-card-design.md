...
**前端只展示入口 Flow 的进度**：Scheduler 可能同时运行多个 Flow（round-robin），但 MessageDock 只关心用户发起的入口 Flow。前端 useSSE 收到 `flow:progress` 时，只有当 `event.sessionId` 匹配当前 `activeSessionIdAtom` 时才更新 `flowProgressAtom`。
...
- 监听 `flow:progress` 事件：仅当 `event.sessionId` 匹配当前 `activeSessionIdAtom` 时更新 `flowProgressAtom`，忽略其他 Flow 的进度事件。
- 监听 `flow:end` 事件：仅当 `event.sessionId` 匹配当前 `flowProgressAtom` 的 `sessionId` 时清空进度（设为 null），避免子 Flow 结束误清入口 Flow 的进度。
