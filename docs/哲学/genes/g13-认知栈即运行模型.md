...
<!--
@referenced-by kernel/src/thread/engine.ts — implemented-by — Thread 即运行中的认知帧
@referenced-by kernel/src/thread/visibility.ts — implemented-by — focus 线程决定可见范围
@referenced-by kernel/src/knowledge/activator.ts — implemented-by — before 帧の KnowledgeRef 解析 + 激活
@referenced-by kernel/src/thread/context-builder.ts — implemented-by — 线程路径 → Context
-->
