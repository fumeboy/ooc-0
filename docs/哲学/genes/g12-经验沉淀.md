...
## 工程映射（2026-04-22，方案 B 完成）

G12 的工程实现通过线程树架构下的 **ReflectFlow 方案 B** 完整闭环：
...
```
  ...
  (bruce 在做任务)                   │ (对象长期记忆)    │
      ...
      │                                         │
      ▼                                         │
 callMethod("reflective/reflect_flow",          │
   "talkToSelf", { message })                   │
      │                                         │
      ▼                                         │
 stones/{name}/reflect/                         │
   ...
   └── threads/{rootId}/thread.json             │
      ...
      │                                         │
      │ ReflectScheduler.triggerReflect(name)   │
      ▼                                         │
 runner（World 注入，跑一轮反思 ThinkLoop）    │
      │                                         │
      │ 反思线程 LLM 决定是否沉淀                │
      ▼                                         │
 callMethod("reflective/reflect_flow",          │
...
| 反思 talkToSelf | `kernel/traits/reflective/reflect_flow/index.ts` 的 `llm_methods.talkToSelf` → `kernel/src/thread/reflect.ts::talkToReflect` |
| 反思线程调度 | `kernel/src/thread/reflect-scheduler.ts` 的 `ReflectScheduler` |
| 沉淀为 memory | `llm_methods.persist_to_memory` → `stones/{name}/memory.md`（append-only） |
...
- 2026-04-22 方案 B 完成调度器 + 沉淀工具 + Context 注入 + 前端适配 + E2E
