...
当前 `think(context="fork")` 默认继承父线程 scope chain。若需要"独立身份"的子线程（如 review 一个未知的 PR、不想被父的 domain knowledge 影响），可以通过 `traits` 参数只声明该子线程需要的 trait——scope chain 从父收集 + 子自定的 traits 合并。

典型场景：想创建一个与父线程能力隔离的子任务，就在 `think(fork)` 时用 `traits` 指定这条子线程需要的完整 trait 清单。
...
| think(fork) traits 参数 | `kernel/src/thread/engine.ts`（处理 think 分支） |
