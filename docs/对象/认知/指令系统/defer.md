...
```
open(title="注册 return 提醒", command=defer, description="提醒 return 前更新摘要")
...
```
open(title="注册 talk 提醒", command=defer, description="提醒 talk 前检查参数")
...
| 概念 | 实现 |
|---|---|
| defer 指令定义 | `kernel/src/thread/commands/defer.ts`（如果实现了） |
