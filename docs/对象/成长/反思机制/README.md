...
> 沉淀不是自动的。需要一个**反思者**来决定"什么值得沉淀"。这个反思者就是 **SuperFlow**。

## 两个文档

| 文档 | 内容 |
|---|---|
| [super-flow.md](super-flow.md) | SuperFlow 作为对象的常驻反思子对象 |
...
`talk(target="super", content="请记住 X")` 并不是"直接把 X 写进 memory"。它是：
...
| 触发 | 机制 |
|---|---|
| 主 Flow 主动 `talk(target="super", ...)` | 立即发消息给 SelfMeta |
...
`verifiable.when_finish` 在 return 前强制验证。SuperFlow 也应该守住**"没有验证就不沉淀"**的底线：

```
talk(target="super", content="请记住：X 方法总是有效")
  ↓
SelfMeta 审视：
  - 用过多少次？
  - 每次都有验证证据？
...
trait "always_use_try_catch_around_llm" 被证明阻止了 failure_to_success sedimentation
  ↓
talk(target="super", content="请移除这个 trait")
  ↓
SelfMeta 审视：
  - 有证据吗？（verifiable 验证过）
  - 影响范围？（仅 LLM 调用）
...
- **合作**（消息）：talk(target="super") 是一次 talk 到 SelfMeta
