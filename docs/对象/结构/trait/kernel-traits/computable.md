...
| `talk_sync(target, message)` | 同步调用其他对象 |
...
每个子 trait 是 Progressive Disclosure 的 Level 3 内容——在需要时通过 `open(type=trait, name=kernel/computable/file_ops)` 按需加载。
...
- **computable + talkable** → 能跨对象调用方法（`talk_sync(target, {method, args})`）
