...
```typescript
reflect("请记住：用 try/catch 包裹 LLM 调用会阻断 failure_to_success sedimentation")
// → 写入 Self memory.md

reflect("请保存：key=preferred_model, value=glm-5.1")
// → 更新 Self data.json

reflect("请沉淀为 trait：...")
// → ReflectFlow 可能创建新的 stone trait
```

`reflect()` 本质上是向 SelfMeta（对象的常驻反思子对象）发消息。SelfMeta 审视后决定是否真正沉淀。
...
只有通过 reflect() 才能写 Self Memory；其他两个 Flow 可以直接写。

### reflect_flow

定义 ReflectFlow 作为一个对象角色应该如何工作：
- 收到 reflect 消息后如何判断"值得沉淀吗"
...
| reflect() 实现 | 本质是 talk("_selfmeta", message) |
