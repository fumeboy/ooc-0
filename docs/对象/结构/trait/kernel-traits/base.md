...
> 唯一的 `when: always` trait。定义 OOC 对象与系统交互的四个原语。
...
**always** 意味着：只要对象的 readme 激活了 base，每次 ThinkLoop 都会注入 base 的内容——这是对象的"基本呼吸"。
...
| `skill` | 加载 skill 内容到上下文 | `name`（skill 路径） |

open 返回 `form_id`，用于后续 submit 或 close。
...
对 command 类型的 form，submit 执行具体指令。args 是指令特定的参数（如 program 的 code、talk 的 target/message）。
...
```
wait(thread_ids?)           ← 等待指定子线程完成
wait()                      ← 等待任意 inbox 消息
```

让当前线程进入 `waiting` 状态，让出调度权。当子线程完成或 inbox 有新消息时自动唤醒。
...
```
open(type=command, command=program, description="...", mark={ id: "msg-123", action: "ack" })
```
