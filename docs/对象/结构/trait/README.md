...
2. **可进化** — 从 readme-only → 默认激活 → 内化为直觉（G12 成长路径）
...
| [渐进式激活.md](渐进式激活.md) | activates_on.show_content_when 驱动的按需激活 |
...
| **Skill** | 技能（按需加载） | 独立的 markdown 内容，`open(title="...", type=skill, name=..., description="...")` 时加载 |
...
Level 3 ── 按需激活（open(title="...", type=trait, name=..., description="...") 或 activates_on.show_content_when 加载完整内容）
```
...
- **基座层** — 系统默认注入，目前只有 `kernel:base`
- **能力层** — 由 `activates_on.show_content_when` 或显式 `open(type=trait)` 按需加载
...
- trait 的**沉淀**（readme-only → 默认激活 / 更高优先级）由 SuperFlow 驱动（[../../成长/反思机制/](../../成长/反思机制/)）
