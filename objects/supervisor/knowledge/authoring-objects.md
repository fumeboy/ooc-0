...
建/改对象进 canonical 都收敛到 feat-branch PR 沉淀模型，权威机制见 reflectable `feat-branch-pr.md`、persistable `stone-pool-flow-three-trees.md`、class `self.md`。
...
stone 持有的逻辑契约是**五件套**（object 与 class 同形态，区别只在 class 不可交互、不跑 thinkloop——见 class `self.md`）：
...
- `exec` 的 `ctx` 是 `MethodExecutionContext`（`method.ts:155`）：`thread? / self?（receiver window）/ args / form?` 等。`ctx.interpreterSelf`（`dir / callMethod / getData / setData / getThreadLocal / setThreadLocal`）**只在 interpreter_process 跑用户代码时由 dispatcher 注入**（`builtins/interpreter_process/executable/self.ts:76`），不是普通 object method 的通用入参。
...
- **继承靠 class**：`package.json` 的 `ooc.class` 声明父类。缺省 `parentClass` 隐式继承 `"root"`（最小 Object 基类，本身只有 example/feishu 边缘 method，**无智能、无工具**）——所以一个只继承 root 的对象是纯工具/数据 Object，不是 agent。**要做 agent**（能被 talk、能思考、跑 thread）须 `ooc.class="_builtin/agent"`：agency（`talk / plan / todo / end`）注册在 `_builtin/agent`，文件/程序/世界/知识等工具能力在它组合持有的 tool-object 成员（filesystem / terminal / interpreter / runtime / knowledge_base）上，经类链/成员窗继承，不必重复声明。prototype chain 已彻底剔除（2026-06-07）。
