...
OOC 把「上下文工程」从 prompt 串接的手艺活，还原成一套面向对象抽象：上下文里每段信息是一个 Object，Agent 本身也是 Object，Object 之间按 OO 经典法则（身份、封装、继承、多态、消息传递）协作。传统 Agent 框架在写一份不断变长的 prompt；OOC 在写一个不断演化的对象图。
