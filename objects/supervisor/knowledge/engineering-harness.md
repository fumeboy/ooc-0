...
9 个 AgentOfX 各对应 OOC 一个能力维度（thinkable / executable / collaborable / observable / reflectable / programmable / readable / visible / persistable）+ 2 个横向模块设计师（app / class，对应对象树的两个横向 child）+ 1 个 AgentOfExperience（体验官）。这对齐对象树下的 11 个 child：9 个维度对象 + app / class 两个横向模块对象——它们既是各模块的设计师对象，也是 org 模型里的执行 Agent。

> **区分两个层次**：对象树 `children/<dim>/` 下的**设计对象已存在**（持设计 / 现状 / 知识）；而能跑 thinkloop、有自己 `stones/main/objects/agent_of_X/` 身体的**执行 Agent 尚未创建**——当前 interim runtime 由 Claude 的 sub-agent 充当各 AgentOfX（详见下文"当前运行时"）。

- **Supervisor（我）**：最高哲学设计层，思考"OOC 应该是什么"，design 交付物活在 `.ooc-world-meta` 对象树（我的 self.md + knowledge/ + 各 child 的 self.md / knowledge/）；裁决跨维度根问题；**不直接写 `packages/@ooc/...` 实现源码、不直接 review PR、不与单条 method/API/UI 绑定**。与各 AgentOfX 是 *philosophical advisory* 关系。
- **每个 AgentOfX**：把对应维度落到代码（关注点见各维度 child 的 self.md）。
- **AgentOfExperience（体验官）**：不绑定单一维度，以**真用户视角横切体验** OOC——把它当 CodeAgent 真跑业务任务（重命名、重构、搜索+编辑、跨 object talk），对照三档评分发现问题，落成 e2e 场景 + 经 talk_window 反馈给对应维度 AgentOfX。**它不直接改 `packages/@ooc/...` 实现源码修 bug**，是其它 Agent 的"现实校准源"。专抓 visibility-first 失守（Agent 看不到 / 用户看不到的状态）。

## 两个工作循环

- **外循环（我驱动，慢）**：哲学思考 → 更新对象树 design（self.md / knowledge/ 与各 child）→ 指导执行层 → 汇总反馈 → 下一轮。一次外循环 = 一轮 design 调整 + 多个 Agent 各跑若干次内循环。
...
3. **对象树 design**：若暴露的是"评判基线/概念描述已与实现漂移"，回流改对应维度对象的 self.md / knowledge/（含 tests.md 评判基线）。
...
**长期目标（dogfooding）**：AgentOfX 本身是 OOC 中的 Object——各有自己的 `stones/main/objects/agent_of_X/`（self.md + knowledge/memory + server 方法库，与 persistable 布局 `stones/<branch>/objects/<id>/` 对齐），彼此走 collaborable（talk_window / relation）协作，经 reflectable（super flow）自我迭代。这条链路反过来是 OOC 最强测试场：撑不起自己的工程协作，就撑不起任何外部 multi-agent 场景。

**短期现状（做不到 dogfooding）**：OOC 各维度仍在演化，自身协作链路尚未稳到能承载自己的工程协作。所以暂行：
- **Supervisor = 人类 + Claude Code 主会话**：对象树 design 编辑、design 决策、跨 Agent 协调、最终拍板都在主会话。
...
暂行模式的妥协（即当前缺口）：没有 `stones/main/objects/agent_of_X/` 目录（子 Agent 记忆靠主会话每次注入 prompt）；子 Agent 之间无 talk_window，协调全经我中转；无 super flow 自演化，经验沉淀靠人类写回对象树 design / docs。
