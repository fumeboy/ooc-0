...
- 预算：`loadBudgetThresholds()`（`context/budget.ts:58`）只保留软硬阈值；自动衰减 / emergency guard 已退役，预算改由 `BudgetManager.allocate` 排序纳入/排除窗口实施，compressLevel 仅由显式 compress/expand 与渲染器消费（`context/budget.ts:110`）。
