...
我负责 OOC 的**思考能力**：一个 Object 如何与 LLM 交互、构造它本轮能看见的 context，并把思考过程组织成可并行、可恢复的 Thread Tree。**knowledge 激活机制不再归 thinkable**（issue N: 整套下沉 builtins/knowledge_base，core 只产 intents 信号、knowledge_base 自决消费）——thinkable 收口为「思考接 LLM」本质。我是 supervisor 之下的维度对象。

**核心边界（thinkable 是 OocClass 模块槽，实现归 builtin）**：thinkable 已升为 OocClass 第五模块（与 executable/readable/persistable 同构，契约 `packages/@ooc/core/thinkable/contract.ts#ThinkableModule`，解析 `resolve.ts#thinkableOf`）。**`packages/@ooc/core/thinkable/` 只留**：`llm/`（LLM client 工厂）+ `context/`（issue N: `scanIntents.ts` 从 contextWindows 聚合 intent 信号）。thinkloop / scheduler / recovery 由 thread builtin 提供（issue H 拆分），context 构造与渲染、事件折入（appendEvents）、每-tick 维护（child-notify）也是 thread 类的 thinkable 实现，物理在 `packages/@ooc/builtins/agent/children/thread/thinkable/`；knowledge 的 parser / activator / loader 整套寄居 `packages/@ooc/builtins/knowledge_base/`（issue N）。core thinkloop/scheduler 经 registry `thinkableOf(thread)` 调用、**不直接 import thread builtin**（唯一例外 `writeThread` = persistable 维度 blessed import）。运行 thread 经 `ctx.ownerThread`（WindowManager.fromThread 注入）供 thread 类载体方法取。
...
- **context**：LLM 的 Input，由若干 ContextWindow 组成，ContextWindow 具有 window methods 可以调用。Object 不知道 context 之外的任何事。总体设计见 `knowledge/context.md`。**issue N**: thinkable 在 context 构造前由 `core/thinkable/context/scanIntents.ts` 从 thread.contextWindows 聚合 intent 信号（`Set<string>`,经 `class.readable.intents?.(self, ref)` 供给）,作为 `ReadableContext.intents` 注入每个 readable render。这是 thinkable 维度对外的**唯一新增产出**——「core 产出意图、ooc class 自决消费」哲学的协议层兑现。
- **thread**：思考过程，可以在思考途中创建 sub threads 创建并行过程。
- **thinkloop**：单 thread 内一轮「构造 context → 调 LLM → 执行 tool → 写事件」循环。

> **knowledge 维度已迁出 thinkable**（issue N, 2026-06-26）—— knowledge 激活规则解析、求值、计算、双源加载、`<knowledge>` 渲染全归 `builtins/knowledge_base/`。thinkable 不再持 knowledge 概念,只产 intent 信号让任意 ooc class（knowledge_base 是实现之一）自决消费。详见 `knowledge_base/self.md`。
...
- knowledge 加载与激活已迁 `builtins/knowledge_base/`（**issue N**）：双源 loader `loadKnowledgeIndex()`（`builtins/knowledge_base/loader.ts`,stone seed + pool sediment，sediment 覆盖 seed）+ 激活算子 `builtins/knowledge_base/activator/` 整套（parser / expr / activator）+ readable 渲染 `builtins/knowledge_base/readable/index.ts`。thinkable 仅在 `buildLlmInput` 里调 `scanIntents` 聚合 intents 集合传给 readable.context（不调 loader、不算激活）；knowledge_base.readable 自渲 `<knowledge>` 子节点（XML 形状从顶层 `<knowledge>` 段变为 `<window class="_builtin/knowledge_base">` 内的 `<knowledge>` 子节点）。trigger 协议简化为**单一 intent 维度**：`intent::<category>::<detail>` 三段式命名空间——`class::<class>`（每条 ref 自动产 full id + short name 双形态） / `form_open::<targetClass>::<guideName>`（method_exec_form 产） / `super_flow::active`（super flow 内 thread 产） / `user::<name>`（用户自定义）；退役 `window::` / `method::` / `super::` 三 kind。
...
最小闭环已落地：LLM 交互、context 构造、intent 聚合、Thread Tree 调度、单轮 thinkloop 全部可跑（core 单测全绿）。knowledge 激活机制已下沉 builtins/knowledge_base（**issue N**）,thinkable 仅产 intent 信号。

历史包袱清理:`object::root` always-on 特判（旧 activator.expr.ts）随 trigger 协议简化退役——issue N 后 trigger 协议改为单一 intent 维度,「root 概念」由 `thread.readable.intents` + `agent.readable.intents` 显式声明产 `class::root` intent（每个 thread 都含 self-view ref + callee agent ref,二者都产 `class::root` → `intent::class::root` 在任何 thread 任何时候命中,等价旧"always-on"语义）。
...
- **activates_on**：knowledge frontmatter 里声明「我何时进入 context」的 trigger map（表达式→级别）。**详见 `knowledge_base/self.md`**——本概念归 knowledge_base 维度,thinkable 不持。
- **trigger**：activates_on 的 key 表达式（**issue N 协议简化为单一 intent 维度**）—— `intent::<category>::<detail>` 三段式命名空间:`class::<class>` / `form_open::<targetClass>::<guideName>` / `super_flow::active` / `user::<name>`。退役旧 `window::` / `method::` / `super::` / `object::`。详见 `knowledge_base/self.md`。
- **intent**（issue N 新增协议元素）:thread 一轮 thinkloop 由 `core/thinkable/context/scanIntents.ts` 从 contextWindows 聚合的 `Set<string>` 信号,作为 `ReadableContext.intents` 注入每个 readable render。命名空间约定见上"trigger"。「core 产出意图、ooc class 自决消费」的协议层兑现。
- **show_description / show_content**：两个激活级别（只露标题描述 / 展开正文）；多 trigger 命中取 max。
- **渐进式知识激活**：执行经 open→refine→submit 渐进暴露窗口与方法,knowledge 随之按 intent 渐进激活——执行到哪、知识激活到哪,控制每轮 context 体积。机制实现归 knowledge_base 维度（详见其 `self.md`）。
...
- **inheritable**：knowledge frontmatter 字段，唯有显式 `true` 才下传给嵌套子 Agent（领域层级轴）。本概念归 knowledge_base 维度。
