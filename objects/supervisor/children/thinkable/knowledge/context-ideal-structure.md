...
description: context 应当如何构成的规范态——分层职责、class/实例边界、预算口径；与 context-construction.md（现状）配对，差距即演化方向
activates_on:
...
> 命名约定：window 的分类一律称 **class**（OOC 已确立 class 是一等继承抽象、唯一继承机制；数据字段本来就是 `window.class`）。`type` 一词只留给 method arg 的**数据类型**（`<arg type="string">`）。现状渲染层把 `window.class` 漂移成 XML 的 `type=` 属性（`xml.ts:320`），属待收敛的债，不是规范。
...
2. **class/实例正确性** = class 级事实只声明一次，实例只携带自己独有的东西。这是 OO 的本分，也是 OOC「Object 化 context」的题中之义——一个 class 的方法表不会抄在它每个实例上。
...
### 2. class 声明层 —— 声明一次（context 优化的核心）

每个在本轮 context 中**出现过的 window class**，其方法契约（method 名 + brief + 必填 `<arg name type required>` 参数契约，见 executable 维度 method-and-constructor）在此层**声明一次**。exec 的调用约定（`exec(window_id, method, args)`）也在此层说一次。

判据：**方法契约是 class 级事实，与具体实例无关，因此每个 class 声明一次，而非每个实例抄一遍。** 实例层只引用 class。

> 这是 P0：现状把 class 级菜单按实例渲染——8 个 knowledge window 各自完整渲染了一模一样的 4 方法菜单，exec hint 那句常量逐 window 重复 14 次。归一化后「首份之外的重复 = 总量 28%」是纯水分。
>
...
每个 `<window>` 只渲染：`id` + `class` 引用 + **实例独有状态**（do/talk 的 transcript、plan 的 steps、todo 的 content、form 的 args/fill_state、self-evolution 的进度）。**不重复方法菜单**（已在 class 声明层）。

判据：**「换一个同 class 的实例，这段会不会变」——会变才进实例层，不变的归 class 声明层。**
...
> 现状 `root-methods` knowledge 用表格列了 do/talk/program… 的用途，而这些 method 的 brief 又在 class 声明层逐条出现——同一批方法的用途说了两次。规范：留 class 声明层的 brief（贴着调用点），删 root-methods 表里与之重叠的部分；root-methods 若仍要存在，只保留「索引 + 何时用哪条」这类菜单 brief 不承载的元信息。
...
把六层落成具体骨架（与 [[context-construction]] 的现状 XML 对照看；注意 window 用 `class=`、arg 用 `type=`，两词不再撞）：
...
<context>
  ...
  <thread id="t_exec_main" status="running">
    ...
    <!-- ② class 声明层：本轮出现的每个 class 的方法契约声明一次；exec 调用约定也只说一次 -->
    <window_classes hint='调用：exec(window_id, method, args={...})。每个 class 的方法对其全部实例可用'>
      <class name="root">
    ...
    <context_windows>
      ...
      <window id="w_talk_peerb" class="talk" status="open">
        <target>peer_b</target>
        <transcript>…</transcript>
      </window>
      <window id="w_plan_1" class="plan"><steps>…steps + 各自状态…</steps></window>
      <window id="w_todo_1" class="todo"><content>…</content><activates_on>…</activates_on></window>
      <window id="f_program_1" class="method_exec" parent="root">
      ...
      <window id="k_forms" class="knowledge"><path>internal/forms</path>…form 推进协议正文…</window>
    </context_windows>
...
**固定协议最小常驻 → class 契约声明一次 → 实例只带自己独有 → 知识按 intent 激活、一个事实一个家 → 预算锚真实渲染成本、降级先于丢弃。** 落到量化目标：真信息占比从 ~26% 拉高，方法菜单从 ~50%（含 28% 重复）降到「每 class 一份」，整体可压缩约 1/4–1/3 而不损失任何 LLM 决策所需信息。

## 与现状的差距（即演化方向）

| 层 | 规范 | 现状（[[context-construction]]） | 差距 |
|---|---|---|---|
| 命名 | window 分类称 class，type 留给 arg 数据类型 | 渲染漂移成 `type=`（xml.ts:320） | type→class 收敛（退役符号流程） |
| 协议 | 仅无条件必需常驻 | 7 条 protocol 多数常驻 | 情境协议下沉 intent 激活 |
| class 声明 | 每 class 一份 | 每实例抄一遍（28% 重复） | **P0：菜单 class 级去重** |
...
骨架（window 对象化、相关性预算、intent 激活）是对的，问题集中在「class 级事实被实例化复制」与「预算估算口径」这两类执行细节——都是熵减型修正，不需要新增机制。
