...
> 与邻接权威的分工（依赖倒置、不复述）：**对象模型**（class/object、继承、单例/非单例、agent 是继承 object base 的 class）见 class `knowledge/object-model.md`；**context 怎么构造成 LLM 输入**见 `context.md`；**thread 是什么 / 数据 / 行为**见 `thread.md`；**thread 怎么调度、单轮 thinkloop、三原语**见 `thinkloop.md`。本文只讲 agent 这一抽象本身。
