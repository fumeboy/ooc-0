...
- **InterpreterSelf 注入**：interpreter ts/js sandbox 与 custom method dispatcher 路径收到 `interpreterSelf = { dir, callMethod, getData, setData, getThreadLocal, setThreadLocal }`（与 method receiver `ctx.self` 区分）。terminal bash 经 env 透出 `$OOC_SELF_DIR`。
...
- object method 类型：canonical `ObjectMethod` 在 `packages/@ooc/core/_shared/types/method.ts:74`（字段构成 + 已删 `match`/`knowledge` 的权威叙述见 knowledge/self-written-method-hot-reload）。自定义 window 声明即 `Partial<ObjectDefinition>`（canonical `ObjectDefinition` 在 `packages/@ooc/core/_shared/types/registry.ts:62`，2026-06-10 删冗余 `StoneObjectDeclaration` 后统一），method 字典字段名为 **`methods`**。
- 动态加载与热更：`packages/@ooc/core/runtime/server-loader.ts:25` `ServerLoader`；`?t=mtime`（:34/:87）破坏 import cache；旧 `llm_methods` 出现即抛硬切错误（:89-93，提示改写为 `export const window … { methods: { … } }`）；独立 `readable.ts` 加载后合并进 `window.readable`（`loadReadableTs`）；接口 `loadObjectWindow`(:109)/`invalidateStone`(:114，按 stone 失效)/`clearCache`(:123)，module-level wrapper `clearServerLoaderCache`(:144)。无 `loadUiMethods` 出口（ui_methods 维度已废）。
- InterpreterSelf：`packages/@ooc/builtins/interpreter_process/executable/self.ts:44` `createInterpreterSelf`（接口 `InterpreterSelf` 同文件 `:9`）；`callMethod`(:52) lookup window → `registry.getObjectDefinition(type).methods[method]` → exec；`getData·setData`(:80,:86) 读写 flow 级 `flows/<sid>/objects/<self>/data.json`；`getThreadLocal·setThreadLocal`(:91,:94)。**已知边界**：`callMethod` 直接索引 `getObjectDefinition(type).methods[method]`，不走 `resolveMethod`（沿 parentClass 链回退，`object-registry.ts:241`）；继承自父 class 的 method 经 ts/js sandbox `self.callMethod` 取不到，只能取本 class 自身声明的 method。
- terminal bash env：`packages/@ooc/builtins/terminal_process/executable/shell.ts:42` `buildBashEnv` 经 `resolveStoneIdentityDir(ref, "write")`(:48) 解析 `OOC_SELF_DIR`(:53)。
- 进程运行时：bash 经 `packages/@ooc/builtins/terminal_process/executable/runtime.ts:16` `runBashExec`；ts/js 经 `packages/@ooc/builtins/interpreter_process/executable/runtime.ts:21` `runInterpreterExec`（经 `createInterpreterSelf`(:39) 注入 self）。
...
- **for_ui_access**（**归 visible**）：`window.methods` 里某方法上的可见性标记；标 `true` 才开放 HTTP `call_method` 通道给人类侧 UI / agent-native（2026-06-11 起替代已废的独立 `ui_methods` 导出——同一份 `window.methods` 表按 `for_ui_access` 过滤）。可见性/通道判定归 visible 维度，不归我；列在这里只为消歧。
- **server-loader / ServerLoader**：动态加载 stone `executable/index.ts` + `readable.ts` 的类，按 mtime 缓存（readable.ts 合并进 `loadObjectWindow` 返回的 `window.readable`）。接口 `loadObjectWindow` / `invalidateStone` / `clearCache`。
...
- **InterpreterSelf**（`interpreterSelf`）：注入进 interpreter ts/js sandbox 与 custom method dispatcher 的能力对象——`{ dir, callMethod, getData, setData, getThreadLocal, setThreadLocal }`。与 method receiver `ctx.self` 是两个东西。
- **`$OOC_SELF_DIR`**：terminal bash 经 env 透出的 stone 目录路径，由 `resolveStoneIdentityDir(ref,"write")` 解析（business session → session worktree object 目录；super/控制面 → main canonical）。
