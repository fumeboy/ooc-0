...
- ProgramSelf：`packages/@ooc/builtins/program/executable/self.ts:51` `createProgramSelf`（接口 `ProgramSelf` 同文件 `:9`）；`callMethod`(:59) lookup window → `registry.getObjectDefinition(type).methods[method]` → exec；`getData·setData`(:88,:95) 读写 flow 级 `flows/<sid>/objects/<self>/data.json`；`getThreadLocal·setThreadLocal`(:104,:107)。**已知边界**：`callMethod` 直接索引 `getObjectDefinition(type).methods[method]`，不走 `resolveMethod`（沿 parentClass 链回退，`object-registry.ts:241`）；继承自父 class 的 method 经 ts/js sandbox `self.callMethod` 取不到，只能取本 class 自身声明的 method。
- program shell env：`packages/@ooc/builtins/program/executable/self-env.ts:19` `buildProgramShellEnv` 经 `resolveStoneIdentityDir(ref, "write")`(:26) 解析 `OOC_SELF_DIR`(:31)。
- program 运行时：`packages/@ooc/builtins/program/executable/runtime.ts:54` `runOneExec` 只路由 shell/ts/js 三种语言模式（旧的 callMethod/function 子模式已退役，见 :10-13）；shell 经 `buildProgramShellEnv`(:69)，ts/js 经 `createProgramSelf`(:91) 注入 self。
