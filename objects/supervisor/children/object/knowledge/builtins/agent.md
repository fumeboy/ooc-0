...
agent 把 agency 收敛在两条 object method，继承本 class 的具体 agent 从此处拿到 agency。**均改 data / 有副作用，均是 LLM-only object method**（无 UI 入口；人机分流移交 visible/server，object method 只管 LLM 侧）：
...
**身份 self.md 的版本化归属有三条互不冲突的路（落三个位置）**：① **`persistable.save`**（agent 改 `data.self`）→ 写**当前 session 的 worktree 副本**（`resolveStoneIdentityRef` business session 落 `flows/<sid>/`，**非版本化、不 commit**，如 git working-tree）；② **人类经 app `PUT /stones/:id/file?path=self.md`** → 直 commit `stones/main`（版本化）；③ **agent 自我迭代经 reflectable feat-branch PR** → 从 main 派生 feat 分支编辑、开 PR 合入。三条写三个不同位置、零冲突。`persistable.save` **不**承担「身份编辑即版本化」语义（A2 不接管 self.md——把 worktree 副本当版本化是语义欺骗）。
...
- **self × reflectable**：自我迭代——经 `talk(target="super")` 进受保护的 super 反思 session，凭 super 投影窗 surface 的 4 个 reflect method（`scan_changes` / `create_pr_for_versioned` / `sediment_unversioned` / `create_pr_for_class_edits`）走 feat-branch PR 沉淀对自身知识/身份/身体的编辑。→ reflectable `self.md`。
...
- **`thread`**（class，非单例）—— agent **一次智能运行的载体**，也是**唯一会话载体注册 class**。`talk` 经其 construct 创建（peer / fork 两形态）。同一 thread 实例按视角（POV）投影成两种 window class（issue E 简化）：`default`（业务 session 内 self-view / peer-view 同框，render 期按 entry.author 区分 prefix）/ `super`（super flow self-view 反思自视，额外 surface reflect method）——投影 class 按视角算出、不持久化。object method：`say`（会话）/ `close`（关 thread 窗）/ `end`（结束本 thread，标 done、记 endReason/endSummary，可选 result 经 creator 窗 say 回报父级）/ `todo`（在当前 thread context 内登记 todo 子对象）+ super 窗 surface 的 4 reflect method（`scan_changes` / `create_pr_for_versioned` / `sediment_unversioned` / `create_pr_for_class_edits`；可见性由 readable.window 控制，method 协议层不再持 `for_reflectable` / `public?` 标签——单一 source = readable.window）。`end`/`todo` 是从 agent agency 迁入的 thread 作用域操作，仅在 `default` 投影窗 surface（`super` 投影窗不 surface）。`wait` 是 tool 原语、作用于窗，不是 thread 的 object method。persistable `mode:"inline"`（整窗随 thread-context 落盘）。
  - **人类发聊天不属 visible/server**：人类经控制面发消息给目标 flow 的 thread，走既有 collaborable 专路 `deliverTalkMessage({source:"user"})`（`app/server/modules/flows`，已存在）——会话派送结构性依赖 live thread，是 collaborable 语义、非「改 object data」。`say` 仍是 thread 的 executable object method（LLM 在 thinkloop 行使会话）；其旧 `for_ui_access` 标记是 vestigial（前端聊天本就不经 callMethod(say)），已随 for_ui_access 退役删去。**visible/server 是纯 object data 编辑**（flow scope），与会话派送正交。
