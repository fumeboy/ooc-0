...
> 它把原先散落在 `class-vs-object.md` / `builtin-addressing-and-instantiate.md` / `class-chain-knowledge-inheritance.md` 的设计逐步吸收进来（吸收完即删，避免分散/漂移）。
...
*(待核心设计定稿后补——`ooc.kind`/`ooc.class`/`ooc.members`/`instantiate_with_new_world` 字段、constructor 注册接口、`_builtin/` 寻址、同名辨析等。吸收 builtin-addressing-and-instantiate.md / class-chain-knowledge-inheritance.md。)*
