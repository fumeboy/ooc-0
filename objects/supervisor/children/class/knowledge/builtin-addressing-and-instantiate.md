...
- **registry**：`_builtin/<id>` 直接作 class 键注册，空 methods 隐式继承 root（`ensureBuiltinClassRegistered`，`packages/@ooc/core/thinkable/knowledge/synthesizer.ts:44`）。ObjectRegistry 的 store 原生支持任意字符串键，无需改数据结构；instance 经 `ooc.class="_builtin/<id>"` 得链 `instance → _builtin/<id> → root`，键不同名，无自引用 break。
