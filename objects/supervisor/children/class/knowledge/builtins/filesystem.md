...
description: filesystem 家族单一权威——单例 tool-object（无 data / 无 construct）；object method grep/glob/open_file/write_file 委托 runtime 造 children file/search 实例；file（非单例 class，open_file/write_file 构造，edit/reload/close + set_viewport/set_range）、search（非单例 class，grep/glob 构造，open_match/close + set_results_window）
activates_on:
...
- **id** `_builtin/filesystem`，**kind = object**（一个具体实例）。
- **单例 tool-object**：parentClass=null（不继承 root）；一个 world 一份文件系统，被多个 agent 组合持有、被 exec 而非被 talk（见 object-model.md 组合）。
- **职责**：对文件世界的统一接入面——grep/glob 搜索、open_file/write_file 打开/写文件，结果以 children 对象（search/file 窗）出现在 context。

## 二、data

`Data = {}`——无业务数据：只承载身份 + 方法面，所有产物都是它造出的 children 对象。
...
- **construct**：无——单例，由唯一规范实例直接寻址、被 agent 组合持有，不 instantiate。
- **visible**：无自定义（filesystem 本体不在控制面单独成页）。
- **persistable**：系统默认（Data 为空，无可序列化实例态）。

## 四、children

children 命名空间从属、各自 parentClass=null、不继承 filesystem（见 object-model.md 核心 8）。
