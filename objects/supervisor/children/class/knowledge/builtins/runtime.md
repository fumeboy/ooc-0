...
> `_builtin/runtime` 是 agent 组合持有的工具对象，向 agent 提供**对象世界语义**的系统级接口——当前唯一方法 `create_object`（建一个全新 OOC Object 的骨架落 session worktree）。对象模型（class/object、单例、继承、组合、children）见 class `knowledge/object-model.md`，本文不复述。

## 一、是什么

- **id `_builtin/runtime`，`kind=class`**，继承 `_builtin/root`。
- **单例 tool-object**（无 construct，一个 world 一份；被 agent 组合持有、被 exec，不被 talk、不跑 thinkloop——见 object-model.md 组合）。
- **职责**：把「agent 操纵对象世界」的系统级原语收口成一个成员窗——当前仅 `create_object`，是后续对象世界元能力（生命周期、类链操作、沉淀治理）的归集点。区别于 `filesystem`（字节层文件）：runtime 操作的是对象世界语义，是元能力面。
...
- **`create_object`**（无 `for_ui_access` → 仅 LLM 可调）—— scaffold 一个全新 OOC Object 的骨架（`package.json` + `self.md` + `readable.md` [+ `knowledge/`]）落**当前业务 session 的 worktree**。
  ...
  - runtime 只做参数校验 + 委派，写盘原语由 persistable 实现（cross-ref persistable `knowledge/stone-pool-flow-three-trees.md` 建对象段 + `session-worktree-model.md` 两条进 canonical 通道）。

### window method

无——runtime 无投影态（无 viewport 等展示态）。

### 投影

固定投影成单一 window class `runtime`，渲染静态 `<about>` 身份/用途文本（系统级接口、create_object 落 session worktree），窗声明展示 `object_methods: ["create_object"]`。无视角分支。

### construct / visible / persistable

- **construct**：无（单例工具对象，不被构造出新实例）。
- **visible**：无 UI。
- **persistable**：系统默认；作为成员窗注入时本就 transient、不落盘。
