...
> 本篇按 sibling **`object-model.md`**（对象模型单一权威）的核心 1–9 给出新建一个 ooc class 的逐文件骨架。
...
├── self.md             # 身份正文（核心 1）
├── types.ts            # data 结构；非单例 class 还在此导出 constructor（核心 1 / 3）
├── executable/index.ts # object method —— 改数据 / 有副作用（核心 5）
├── readable.ts         # 投影成 context window + window method（核心 4）
├── visible/index.tsx   # 可选：UI（核心 6）
└── persistable/index.ts# 可选：自定义序列化（核心 7）
```
...
- **`ooc.kind`** —— 这份 stone 是 **class**（一份定义/模板）还是 **object**（一个具体实例）。`note` 是定义 → `"class"`；它被构造出来的实例（运行时窗）是 `"object"`。
  - 单例 / 非单例是**另一条轴**（核心 3）：是否导出 constructor。`note` 有 constructor → **非单例 class**（按需造多个实例）。单例 class 无 constructor、有唯一规范实例。两者 `kind` 都是 `"class"`。
- **`ooc.class`**（可选）—— 继承谁（核心 2），值为父类 id，如 `"_builtin/agent"`；不写则隐式继承基类。是**单链**继承。
...
## types.ts —— data 结构 + constructor（核心 1 / 3）
...
export interface NoteWindow extends BaseContextWindow {
  class: "note";
  status: "open" | "closed";
  body: string;            // 业务数据：object method 读写（核心 5）
  // 展示状态（viewport 等）归 state，readable 的 window method 读写（核心 4）
}

// 非单例 class 的 constructor（核心 3）：产出一个新 object 实例。
...
## executable/index.ts —— object method（核心 5）
...
## readable.ts —— 投影 + window method（核心 4）
