...
`write_file`/`edit` → session worktree → super flow `evolve_self` commit+merge main → 下一轮新 thread 见新身份/新方法。stone executable 还能 mtime 热更（`packages/@ooc/core/runtime/server-loader.ts:78` 动态 `import(...?t=mtimeMs)`）。

证据：2026-06-06 persistable harness **Good**——agent 改 self.md → worktree → `evolve_self`（`evolve-self.ts:124`）→ main → 新 session 见新身份。A 的循环真的能闭。
...
| 3 | **治理** | `programmable/versioning.ts`（scope/merge/rollback） | scope/merge/rollback 模型是 stone 形状（self-scope ff-merge / cross-scope PR-Issue）。核心源码「无主」，没有 Object 拥有它，scope 模型不适用。 |
...
把 AgentOfX 的 stone 设成**框架源码的一个切片**（如 AgentOfThinkable 的 stone = thinkable 源码），B 就归约为 A：「Object 迭代自己的 stone，只是这个 stone 恰好是框架源码」。**ownership = scope**，evolve_self / 治理直接复用。三缺口因此收敛为两个真问题：
...
| 5 governance | talk(super)→evolve_self | —— | **✗**（core 无主，scope 不覆盖） |
