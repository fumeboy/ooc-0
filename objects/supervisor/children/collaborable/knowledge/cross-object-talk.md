...
`say` 的细节：`talk_window.say` 是挂在 talk_window 上的 window method（`sayMethod: ObjectMethod`，不再叫 "command"），`wait=true` 时父线程进 `status="waiting"`，等对端回复进 inbox 唤醒（`packages/@ooc/core/executable/windows/talk/method.say.ts:99`，wait 行为见 `:91` exec 体 / `:20` 知识注释）。
