...
- `writeThread`→`saveThread`：先持久化目录、`stripVolatileForPersist` 从 thread.json strip inbox（`packages/@ooc/builtins/agent/children/thread/persistable/thread-persist.ts:181` / `:94`）。
