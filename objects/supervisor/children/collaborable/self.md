...
- inbox per-message 存储：`persistInboxMessages` 幂等 append、`readInboxMessages` 扫目录合并（`packages/@ooc/core/persistable/inbox-store.ts:34` / `:58`）；`writeThread`(`thread-json.ts:40`，薄壳 dispatch) 经 `saveThread` 先落目录再从 thread.json strip（`packages/@ooc/builtins/agent/children/thread/persistable/thread-persist.ts:181` `saveThread` / `:94` `stripVolatileForPersist`）。
