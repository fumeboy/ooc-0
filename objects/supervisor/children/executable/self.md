...
2. **Method 层** — do / talk / program / plan / todo / end / open_file / open_knowledge / write_file / glob / grep / metaprog（治理：resolve/rollback）等具体行动。2026-05-28 起 Command 与 Object Method 归一为 **Method**（`object.doc.ts:751`）。
...
最小闭环已落地。最近一次迭代：write / edit / program-shell 三通道收敛到 **session worktree** 统一模型——business session 对任意 stone（含 cross-object）的读写重定向到 `flows/<sid>/`（eager checkout main 全量，裸读可见全量），super flow `evolve_self` commit→`tryMergeSelf`（self-scope ff-merge / cross-scope PR-Issue）才永久（`docs/2026-06-09-remove-metaprog-unify-session-worktree-design.md`）。program shell `$OOC_SELF_DIR` 经 `resolveStoneIdentityDir` 解析（`packages/@ooc/core/executable/program/self-env.ts:16`）。

## 已知问题 / 边界与未决

- **compress 仅 scope=windows 落地**，scope=events/auto 抛 not-implemented（`packages/@ooc/core/executable/tools/compress.ts`）。
- **自改命令集无硬 deny**：自改 `stones/<self>/executable/index.ts` 仅靠 write_file 弱 ask，缺硬拦（doc Q0e，`object.doc.ts:1080`）。命令集 / readable 为全局 main-canonical，per-session 改须经 evolve_self 合入 main 后重注册才生效。
