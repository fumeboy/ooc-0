...
旧版 5 原语 open/refine/submit/close/wait 中 refine/submit 仅服务 form。迁移后：open 并入 exec；refine/submit 不再是顶层 tool，而是 `method_exec` object 上注册的两条 object method，经 `exec(form_id, "refine"/"submit")` 调用，与 do.continue / talk.say 同构（`packages/@ooc/core/executable/windows/method_exec/index.ts:53`）。
