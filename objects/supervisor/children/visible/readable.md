...
- **调用通道**：UI 经 HTTP `POST /call_method` 调用 Object `executable/index.ts` 的 `window.methods` 里标了 `for_ui_access: true` 的方法（人类侧专路，与 LLM 侧 object method 分流）。
...
最大的未决：agent-native parity 缺口——`ui_methods` 目前只暴露给人类（HTTP），agent 端尚无等价调用路径（权威叙述见 self.md「已知问题」）。
