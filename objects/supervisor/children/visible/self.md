...
- **P1 builtin 真 UI 落地**(2026-06-29, commit `3288a04d`):8 个核心 builtin window type(file / knowledge / todo / search / skill_index / plan / program / root)在 `web/src/domains/files/components/visible/` 实装真 read-only UI 组件,与既有 method_exec / feishu_chat / feishu_doc / talk 同结构;`BUILTIN_VISIBLE` 注册表 8 槽位全部从 Placeholder 切到真组件,控制面渲染 thread context 时不再"全是 JSON"。`PlaceholderWindowDetail` 改 named export 留作未来兜底。测试 19 pass(含 `builtin-visible-render.test.ts` 用 `react-dom/server.renderToStaticMarkup` 给每个组件喂最小 window 渲染烟雾测)。web build 2047 modules / 937KB / 8.78s。issue:`docs/issues/2026-06-29-p1-builtin-visible-from-placeholder.md`。**仍是静态注册表路径**,Phase 2(builtin 自带 `<ObjectDir>/visible/index.tsx` + `client-source-url` endpoint 实装 + 切 dynamic)未启动。
- **P1 后续 cw-* 视觉规则**(2026-06-29, commit `c647ddf4`):P1 acceptance review 顺手发现 #1 落地。8 个 P1 组件引入的 14 个 `cw-knowledge-* / cw-search-* / cw-plan-* / cw-program-*` className 在 `web/src/styles.css` 补齐对应规则;新增通用 `<span data-status>` 自动 badge 化(10 种状态色);设计语言对齐既有 `cw-feishu-msg-*` / `cw-skill-*` / `cw-feishu-doc-*`。CSS bundle +约 1KB,build 仍 2047 modules / 8.81s。
- **root window 归属裁决**(2026-06-29, 用户拍板):root window **不作为 ooc class 存在** — root 是 thread context 的 anchor、非 ooc object。Phase 2(P2 issue)实装 builtin 自带 `visible/index.tsx` 时**不为 root 建 builtin**,`RootWindowDetail.tsx` 保留在 `web/.../visible/` 作 web 内部组件,`resolveWindowVisible` 加显式特例分支识别 `window.class === "root"` 走内部组件、不走 dynamic endpoint(否则 endpoint 必返 404)。Phase 2d 退潮时**保留 RootWindowDetail.tsx**、删其余 7 个 web 镜像 tsx。
- **P2a endpoint 落地**(2026-06-29, commit `25b29c03`):新建 `modules/ui/` server module,首位住户 `GET /api/objects/:scope/:objectId/client-source-url`。后端权威给出 visible 源码的 `{absPath, fsUrl}`,前端 dynamic import 不再自拼路径。stone canonical / session worktree (`?sessionId`) / `?file=diff` / legacy fallback / flow 多页 / builtin 框架包路径 6 条解析全打通,4 层安全防穿越。Tests p2a-client-source-url 15 pass。visible "Object 持有并演化自身 UI" 通路的 endpoint 兑现。
- **P2b builtin 自带 visible 落地**(2026-06-29, commit `98dce7a5`):7 个 builtin (file / search / knowledge / todo / plan / skill_index / interpreter_process) 在 `<ObjectDir>/visible/index.tsx` 实装 UI + `<ObjectDir>/visible/server/index.ts` 装配 VisibleServerModule。可编辑 builtin (todo / plan) 实装真 method 共 8 个;read-only 5 个空 module 留位。class 装配 `visible: visibleServer` 与 thread S2 范例一致。styles.css 补 `cw-btn` / `cw-todo-*` 规则。Tests p2b-builtin-visible-server-wiring 11 pass,P2a+P2b+S2+S1+registry 46 pass 0 fail。**仍走静态注册表渲染** (web 端组件未删,resolveWindowVisible 未切 dynamic);Phase 2c 才切路径、Phase 2d 删 web 镜像。
- **P2c 路径切换 + callMethod 接通**(2026-06-29, commit `61a5ac6e`):`resolveWindowVisible.tsx` 默认 dynamic + 新增 `kind="root"` 特例;`ObjectClientRenderer` 删 `TODO_async` 桩接通 P2a endpoint;`WindowVisible` 入口注入 `callMethod` prop (sessionId 在时,经 `flowCallMethod` endpoint dispatch 到 visible/server)。plan UI 补真交互。**重大顺手修**:`resolveBuiltinDir` 经 `nestedObjectPath()` 翻译 OocClass 逻辑 id → npm 物理路径,修嵌套 builtin 全部解析失败的潜在 bug。
- **P2d 退潮**(2026-06-29, commit `7e33faad`):删 7 个 web 镜像 tsx (`{File,Knowledge,Todo,Search,SkillIndex,Plan,Program}WindowDetail.tsx`),`BUILTIN_VISIBLE` 静态表 12 → 4 槽 (剩 `method_exec / feishu_chat / feishu_doc / talk` P3 候选;root 走特例)。**bundle 缩小信号**:dist/main 937KB → 924.86 KB。Tests 226 pass / 0 fail (后端 215 + web 11)。
- **P2 整体兑现**:visible 维度 "Object 持有并演化自身 UI" 的完整通路从设计承诺变机制层既成事实 — endpoint (P2a) + builtin self-hosted (P2b) + 路径切换 + callMethod 接通 (P2c) + 退潮闸门 (P2d) 全部到位。控制面渲染 thread context 时,7 个核心 builtin 经 dynamic 路径加载它们**自己拥有**的 visible/index.tsx;可编辑 builtin (todo/plan) 可经 callMethod 调自己的 visible/server method 改 data。**待 acceptance review**:本 issue 体量大 (4 阶段 / 8 commit),建议后续派 reviewer 独立验收转 verified。
- **P3 死键修正 + 死类型组件清退**(2026-06-29, commit `e97aa88c`, issue `docs/issues/2026-06-29-p3-dead-keys-and-types-cleanup.md`):BUILTIN_VISIBLE 静态表 4 槽 → 1 槽。深度勘探发现剩余 4 个槽位的 3 个 (`feishu_chat / feishu_doc / talk`) 是 ooc-6 旧设计的 web 端类型镜像,后端**无任何代码**生产这些 class 字面值 — 死类型。`method_exec` 是 key 命名漂移 (前端短名 vs 后端 `_builtin/agent/method_exec_form` 全名) → key 改全名对齐。3 个死类型组件文件清退,BUILTIN_VISIBLE 仅留 method_exec_form 一项 (form UI 强依赖 CodeMirror 等 web 基础设施,迁 builtin 违反 P2b 裁决,留 web 端合理)。**bundle 再缩**:dist/main 924.86 → 918.98 KB (累计 vs P1 落地 -2%)。**仍待清理**:web 端 ContextWindow union 三个死类型分支 + 43 处消费方引用 (P4 候选独立 issue)。
- **P4 context-snapshot dead union 清退**(2026-06-29, commit `e46af0c2`, issue `docs/issues/2026-06-29-p4-context-snapshot-dead-union.md`):web 端 `ContextWindow` union 删 4 个死类型分支 (`do / talk / feishu_chat / feishu_doc`),context-snapshot.ts / chat/model.ts / ContextSnapshotViewer / SessionThreadsIndex / chat/formatter / RightPanel 6 文件死代码清理。helper (`filterMessagesForDoWindow / filterMessagesForTalkWindow`) / type alias (`DoWindowShape / TalkWindowShape`) 全部删,window-scoped transcript 视图归并到顶层 inbox/outbox 兜底。6 files / **-238 行 / +33 行 / 净减 -205 行**。bundle 再缩 918.98 → **916.31 KB** (累计 vs P1 落地 -2.2%)。仍保留:同名但不同语义的 `RelationKind="talk"`(agent 间 relation)、`message.source==="talk"`(消息来源字段)。
- **P5 InlineTalkComposer / prop 链清退**(2026-06-29, commit `2631de15`, issue `docs/issues/2026-06-29-p5-onuserreply-prop-chain.md`):P4 后续退潮 — InlineTalkComposer 函数 + `onUserReply / selfObjectId` 两个 prop 跨 8 文件 (ContextSnapshotViewer / FileViewer / ThreadDetailTabs / ThreadInspectDetail / UserThreadHome / SessionThreadsIndex / MainPanel / shell) 传递链整体清退。talk window 退役后,这条专为 talk 内联回复 UI 服务的 prop 链已经永远不渲染。8 files / **-116 行 / +18 行 / 净减 -98 行**。bundle 缩 916.31 → **915.86 KB**。用户回复经 RightPanel.ChatComposer → handleSend 路径独立保留,本退潮不影响用户体验。
- **累计 P1 → P5 退潮信号**:5 phase 连续退潮闸门关上,web 端 bundle 累计 **-21.14 KB / -2.26%**,从 talk window 类型 → union 镜像 → InlineTalkComposer 函数 → prop 链 4 层 dead code 全部清干净。剩余技术债:ThreadInspectDetail 孤儿组件 + plan shape 漂移 + ObjectClientRenderer vs resolveWindowVisible 实现统一 + addUserTalkWindow endpoint 验活,留独立小 issue。
