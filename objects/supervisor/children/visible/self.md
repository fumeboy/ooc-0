...
- callMethod 路径**不在** ui module，而是 `app/server/modules/stones/service.ts:387` `callMethod`（+ `modules/flows/service.ts` 对端）：`loadUiMethods(ref)` →（`runtime/server-loader.ts`）`uiMethods[method]`；错误码 `METHOD_LOAD_FAILED`（`stones/service.ts:394`）/ `METHOD_NOT_FOUND`（`:402`）。HTTP 入口 `POST /api/stones/:id/call_method`（`modules/stones/api.call-method.ts:6`）。
