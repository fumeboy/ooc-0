...
- 路径计算：`packages/@ooc/core/persistable/common.ts:47` objectDir / `:57` threadDir / `:72` stoneDir / `:131` resolveStoneDir（2-path：flat `stones/<id>/` → versioning `stones/<branch>/objects/<id>/`；deprecated packages/ 兼容层已于 2026-06-07 移除）；pool 在 `pool-object.ts:54` poolDir。
