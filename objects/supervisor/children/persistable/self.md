...
**建新对象 vs 改已存在对象**（口诀，2f4456f9）：建新对象骨架 = `create_object` root method（`createObjectInSession`，`stone-create-object.ts:94`）；改已存在对象文件 = `write_file` / `file_window.edit`。口诀与落点细节（落 `flows/<sid>/objects/<newId>/` 不 commit、双重 ALREADY_EXISTS 校验、session 内当场可用、进 canonical 走 feat-branch PR）权威在 `knowledge/stone-pool-flow-three-trees.md`。
...
- PR-Issue 存储与决议：`packages/@ooc/core/persistable/pr-issue.ts:62` `PrIssueRecord`（reviewers/approvals）/ `:301` `createPrIssue` / `:119` `aggregatePrApproval` / `:451` `approvePrIssue` / `:390` `closePrIssue`；合入复用 `stone-versioning.ts:291` `resolvePrIssue`；HTTP 直写 `stone-versioning.ts:561` `httpDirectMainWrite`；建对象 `persistable/stone-create-object.ts:94` `createObjectInSession`。
...
- thread-context 落盘（§10 单点刷，b24ba0ef）：`packages/@ooc/core/persistable/thread-json.ts:40` `writeThread`（薄壳 dispatch 到 `packages/@ooc/builtins/agent/children/thread/persistable/thread-persist.ts:181` `saveThread`）是**唯一**持久化入口，单点刷 `thread-context.json`，自动覆盖所有绕过 WindowManager 直改 `thread.contextWindows` 的写路径（delivery / thinkloop reconcilePeerWindows / scheduler / worker）；entries 由 `packages/@ooc/builtins/agent/children/thread/persistable/thread-persist.ts:67` `buildEntries`（唯一生成规则）产出。thread.json.contextWindows[] 字段已完整退役，旧数据若仍含一律忽略。
...
thread 容器持久化下沉 + 退 container indirection（2026-06-16）：thread 的 thread.json/thread-context/inbox/hydrate 逻辑在 thread builtin（`builtins/agent/thread/persistable/thread-persist.ts`），走 **object-model 标准 `persistable.save/load`**——同日先引入又退役了专属 `PersistableModule.container = {write,read,writeSnapshot}`（为实现偶然性套命名，错的）。`PersistableContext` 加可选 `threadId` 供 thread 二级寻址；`writeThread`/`readThread` 退为 thread 作用域薄 API 经 registry dispatch 到 save/load；manager `reportContextEdit` 同步实例 map 后整份 writeThread（取代 writeSnapshot）。归属原则：窗持引用+视图态、对象持数据、消息属 thread 对象层（不内联被指对象数据）。pr-issue/git/stone 经判据留 core 框架。同期 `isCreatorWindow` 去状态化（creator 窗身份编码在 id=`creatorWindowIdOf`，删 data flag，消费方按 `isCreatorWindowId(id)` 派生 / close 不可关靠投影可见性）。详见 `knowledge/core-framework-vs-builtin-logic.md`。
