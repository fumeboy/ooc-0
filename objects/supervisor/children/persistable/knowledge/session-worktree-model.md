...
- **super-flow evolve_self = 身份合入闸门** ⚠️ **本条已退役（2026-06-11 用户拍板）**：session 分支**永不合并回 main**；`tryMergeSelf`/self-scope ff/cross-scope PR-Issue/`evolveSelfMerge` 已删（commit 2735241c）。沉淀进 canonical 改走 **feat-branch PR**（super(foo) 另起 feat 分支编辑 → `evolve_self` finalizer commit+PR → reviewer 冒泡审批），权威 `docs/2026-06-11-reflectable-feat-branch-pr-flow-design.md`，P3-P6 完成后全面回流。下述退役描述暂留作历史参照：~~把某业务 session worktree 改动正式合入 main——commit `session-<sid>` 分支 → rebase → `tryMergeSelf` 分类：self-scope ff-merge 回 main / cross-scope 转 PR-Issue → supervisor `resolve`~~。合入后 GC（`gitWorktreeUnregister` 解 `.git` link、保留 `flows/<sid>` 运行时数据）的机制 feat-branch 流程复用。
