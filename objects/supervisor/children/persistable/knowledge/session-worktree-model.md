...
- **LLM 演化**：业务 session worktree → super-flow `evolve_self` 合入 main（self-scope ff-merge / cross-scope PR-Issue）。
- **HTTP 控制面写入**：`putSelf` / `putServerSource` / `createStone` 直接 commit main，立即生效，不开 worktree。
