...
- **动态面**经 `registerReadable` 注册：`readable`（按状态算 XML，优先级高于 readable.md）、`windowMethods`（控展示的方法，如 set_viewport，只动展示状态不碰业务数据）、`compressView`（折叠/快照态）、`onClose`、`consumedMessageIds`。与 executable（object method 改业务数据）按维度分注册、互不覆盖、同名 fail-loud。
