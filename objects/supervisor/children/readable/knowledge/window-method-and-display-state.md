...
**WindowDisplayState**（`packages/@ooc/core/_shared/types/window-state.ts:8`）是纯展示字段容器：`viewport` / `lines` / `columns`（file/knowledge 的行列视口）、`transcriptViewport`（talk/do）、`resultsViewport`（search）、`historyViewport`（program）。**只放展示参数，不放业务数据**（file path、program history 这些归 window 业务字段）。它随 window 落 thread-context.json（thinkable §10 后 contextWindows 的唯一权威落点）。
