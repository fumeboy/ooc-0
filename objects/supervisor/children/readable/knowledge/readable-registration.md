...
- `registerExecutable(type, patch)`（`:113`）——只收 `methods`（object method）+ 类元 `parentClass` / `isBuiltinFeature`。
- `registerReadable(type, patch)`（`:128`）——只收 readable 维度字段：`readable` / `windowMethods` / `compressView` / `onClose` / `consumedMessageIds`（`packages/@ooc/core/runtime/object-registry.ts:128-133`；字段定义见 `_shared/types/registry.ts:62-79`）。旧 `renderXml` 已并入 `readable`、`basicKnowledge` 已退役（registry.ts:56-57）。
...
标准对象定义样板：`packages/@ooc/builtins/example/`（self.md + executable/index.ts + readable.ts，barrel 分注册）。

**沿 class 链回退**：registry 有 `resolveWindowMethod`（window method 沿 parentClass 链回退，镜像 object method 的 `resolveMethod`，`object-registry.ts:257/261`）；当前无自定义对象覆盖 readable，该链尚未被真正行使——详见 self.md「已知问题」。
