...
- **stones**：stone 五件套读写。**所有写 stone 的 HTTP 必经 stone-versioning**（worktree → commit → merge），无 uncommitted 半成品。（裁定：stone scope **无 call_method**——不调 object 程序、stone client 只读，运行时/data 编辑归 flow scope；代码侧 stones `/call_method` 待退役。）
