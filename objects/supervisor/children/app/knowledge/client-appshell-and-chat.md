...
`packages/@ooc/web/src/app/routing.ts:42` 定义 `RouteState` 六个 kind：welcome / scope / file / stoneClient / flowPage / flowsView。`toPath(state)`（`:79`）是 URL 反向构造、`parseRoute(pathname, search, params)`（`:160`）是正向解析、`useRouteState()`（`:140`）从 react-router 当前 URL 派生 RouteState。

`shell.tsx` 不 `setState` 改导航字段——所有 handler 走 `navigate(toPath(...))`，URL 变化经 `useRouteState` 回流为下一帧 state。这让前进/后退、刷新、URL 复制粘贴都能恢复页面。导航维度全部从 URL 派生：`activeSessionId`（`shell.tsx:57`）、`activeObjectId`（`:76`）、`activeThreadId` / `activePath`。本地 `useState` 只承担两类职责：数据缓存（tree/thread/flows/stones + hash）与 transient UI（表单 draft、modal、showSessions）。
...
- **polling-job 协议**：发起动作拿 `jobId` → 轮询 job 状态（`domains/chat/policy.ts` 的 `waitForJob`，约 10s 上限）→ job 终态后刷新 thread。**无 SSE**。额外有 4s thread 静默轮询（`shell.tsx:192`）做 hash diff 后只在变化时更新，让 callee 在用户不动时也渐进显示新事件。
...
- **tree-scope**：flows / stones / world 三 scope 的目录树 + 文件预览。
...
- **写入边界**：client 唯一开放写入是 `stones/{objectId}/knowledge/**`，其余只读浏览，不让 UI 旁路 server 策略。
